...
vector3 recursiveTracer (vector3 i, vector3 contribution) {
    ...
    vector3 l = wall.intersect(i, lightPosition);
    
    if ((c.x >= -25.0) && (c.x <= 25.0) && (c.y >= 0.0) && (c.y <= 50.0)) {
        contribution = localColor(c, lightRed, wallNormal);
    } else if ((l.x >= -25.0) && (l.x <= 25.0) && (l.y >= 0.0) && (l.y <= 50.0)) {
        vector3 shadowColor = localColor(l, vector3(0, 0, 0), wallNormal);
        contribution = color.scalar(local_coef).add(shadowColor.scalar(reflect_coef));
